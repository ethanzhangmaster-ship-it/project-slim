"""Phase 2.2A: Queue Metrics — queue depth, wait times, oldest task.

Reads from core GenerationStore (read-only). Tracks:
  - Queue depth by status
  - Oldest pending task
  - Average/max wait time

No Event Bus subscription needed — polled by Dashboard on demand.
"""
...
class QueueMetrics:
    ...
    def wait_times(self) -> dict[str, float]:
        """Average and max wait time for pending tasks."""
