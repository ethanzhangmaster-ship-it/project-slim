...
class CreativeSync:
    def __init__(self):
        self._platforms = ["meta", "tiktok"]
        self._sync_records: Dict[str, CreativeSyncRecord] = {}
        self._sync_history: List[Dict[str, Any]] = []

    def sync_creatives(self) -> Dict[str, Any]:
        results = {}
        for platform in self._platforms:
            result = self.sync_platform_creatives(platform)
    ...
    def sync_platform_creatives(self, platform: str) -> Dict[str, Any]:
        if platform not in self._platforms:
            return {"success": False, "message": f"Unknown platform: {platform}"}

        record = CreativeSyncRecord(
            platform=platform,
