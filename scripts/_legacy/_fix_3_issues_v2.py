"""修复3个问题：广告主、文案、图片（去掉不支持的title/subtitle）"""
...
for i, img_info in enumerate(manifest['images'], 1):
    ...
    with open(img_path, 'rb') as img_file:
        r = requests.post(
            ...
            data={"access_token": USER_TOKEN, "filename": f"p04-fixed-{i}.png"},
...
print(f"\n【步骤 4/5】创建 Creatives（只有 message，去掉 title/subtitle）...")
ad_messages = [
    "Merge Witches - The most addictive merge puzzle game! Merge witches to unlock powerful spells and magical abilities. Download now and start your magical journey!",
    "Discover the magic of Merge Witches! Combine cute witch characters and create powerful magical beings. Over 100 levels to explore!",
    "Merge Witches - A magical puzzle adventure! Merge, collect and evolve your witches. Join millions of players worldwide!",
    "Experience the enchanting world of Merge Witches! Merge magical creatures and build your witch kingdom. Hours of fun!",
    "Unleash your inner witch with Merge Witches! Merge characters, cast spells, and conquer magical challenges. Free to play!",
]

creative_ids = []
for i, (image_hash, message) in enumerate(zip(image_hashes, ad_messages), 1):
    r_cre = requests.post(
        f"{BV}/act_{ad_account_id}/adcreatives",
        data={
            "access_token": USER_TOKEN,
            "name": f"P04-AI-Fixed-Creative-{i}",
...
print("3. 添加了广告文案(message)")
print("4. 使用了新图片")
print("\n⚠️  AEO购物事件需要配置 pixel 和转化事件")
print("当前账户没有 pixel，请在后台配置后再改为 AEO")
