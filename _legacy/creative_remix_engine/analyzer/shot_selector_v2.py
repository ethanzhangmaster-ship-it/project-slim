"""Shot Intelligence Selector V2 — V3.6.1 Ranking Engine Calibration 版

升级点：
- 使用 V2 Scorer（hook_score_v2, gameplay_clarity, role_scores, ad_value）
- Beat-specific Ranking 权重
- 多标签角色匹配（不再依赖单一 content_type）
"""
import json
import subprocess
...
class ShotSelectorV2:
    """V2 Shot Selector — V3.6.1 Beat-specific Ranking"""
    ...
    def __init__(self, game_code: str = "P04",
        ...
        if ranking_db_path is None:
            ranking_db_path = Path("d:/project_slim/project_slim/creative_remix_engine/storage/outputs/v35_ranking_db.json")
    ...
    def build_pool(self, source_dir: Optional[Path] = None,
        ...
        print(f"[ShotSelectorV2] Pool: {len(candidates)} shots | Ranking hits: {sum(1 for c in candidates if c.hook_score_v2 > 0)}")
        return candidates
    ...
    def select_for_beat(self, beat_role: str, beat_emotion: str,
        ...
        """为单个 Story Beat 选择最佳镜头（Beat-specific 权重）"""
